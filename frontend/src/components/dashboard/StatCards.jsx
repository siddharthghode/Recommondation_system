...
function StatCards({ stats }) {
  const cards = [
    {
      label: "Total Books",
      value: (stats.total_books ?? 0).toLocaleString(),
      subLabel: "In stock",
      subValue: stats.in_stock ?? 0,
      icon: BookOpen,
    ...
    },
    {
      ...
      value: (stats.active_borrows ?? 0).toLocaleString(),
      subLabel: "Returned",
      subValue: stats.returned_today ?? 0,
      icon: BookCopy,
    ...
    },
    {
      ...
      subValue: stats.active_borrows > 0 ? `${((stats.overdue_count / stats.active_borrows) * 100).toFixed(1)}%` : "0%",
      icon: AlertCircle,
    ...
    },
    {
      ...
      subValue: Math.round((stats.returned_today ?? 0) * 0.35),
      icon: RefreshCw,
